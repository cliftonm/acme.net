namespace Oocx.ACME.Console
{
	...
	class ContainerConfiguration
	{
		public IContainer Configure(Options options)
		{
			...
			else if ("xsp-http-01".Equals(options.ChallengeProvider, StringComparison.OrdinalIgnoreCase))
			{
				builder.RegisterType<XspChallengeProvider>().As<IChallengeProvider>();
			}
			else
			{
